#!/usr/bin/env python3
"""Generate colour.html, Colour: the eye, the gamut, and the names.

Three views. The eye: the spectrum with the sensitivity curves of the three
cones and the rod drawn over it, and a marker that drags along the
wavelengths and reads how much each receptor answers. The gamut: the CIE
1931 chromaticity diagram, every colour the eye can see as a horseshoe
computed from the standard observer, with the triangle a screen can show,
the black-body curve, the daylight white, and a marker that reads any
point's coordinates and dominant wavelength. The names: the eleven basic
colour terms in the order languages acquire them.

Data: tools/colour_data.py.

Usage: python3 build_colour.py
"""
...
import apa
from colour_data import RECEPTORS, COUNTS, SRGB, POINTS, TERMS, STAGES, REFS

OUT = Path(__file__).parent.parent / "colour.html"

NOTE1 = ("Colour is not in the light. Light has a wavelength; colour is "
         "what a brain makes of the answers of three kinds of cone, tuned "
         "to short, middle and long waves, and every hue anyone has seen "
         "is a mixture of three numbers. The first view is those three "
         "curves, with the rod that works at night, over the spectrum; the "
         "marker reads how each answers to any wavelength.")

NOTE2 = ("The second view is the map of everything the eye can see, the "
         "horseshoe of the 1931 diagram, with the small triangle inside it "
         "that a screen can show, the curve of glowing things from red-hot "
         "to blue-white, and the white of daylight at the centre; the "
         "marker reads any colour's place. The third is what languages do "
         "with all this: eleven names, acquired in nearly the same order "
         "everywhere, dark and light first, red next, blue late.")

METHOD = ("The receptor curves are drawn as bells at the peaks Bowmaker and "
          "Dartnall measured in a human retina, 420, 534, 563 and 498 nm, "
          "with widths chosen to match the published sensitivities; they "
          "are not the measured curves point for point, and the lens and "
          "the yellow spot in front of the retina shift the effective "
          "peaks a little. The horseshoe is computed from the CIE 1931 "
          "two-degree observer in Wyman, Sloan and Shirley's analytic fit, "
          "between 420 and 640 nm, closed with the observer's tabulated points "
          "at 380, 660 and 700 nm where the fit's tails are unreliable; its "
          "fill is the nearest colour a screen can show, so "
          "everything outside the triangle is shown less saturated than it "
...
<title>Colour &middot; Altazor</title>
<style>
...
#numTxt .sw { display:inline-block; width:14px; height:14px; border-radius:3px; vertical-align:-2px; margin-right:4px; border:1px solid #3d3d3d; }
...
<header class="site">
  ...
  <nav class="site"><a href="library.html">&larr; Library &middot; Abstractions</a><a href="light.html">Light</a><a href="brain.html">The Brain</a><a href="languages.html">Languages</a></nav>
</header>
<h1>Colour</h1>
...
function xyzToLin(X,Y,Z){ return [3.2406*X-1.5372*Y-0.4986*Z, -0.9689*X+1.8758*Y+0.0415*Z, 0.0557*X-0.2040*Y+1.0570*Z]; }
...
function fit(rgb){ let [r,gg,b]=rgb; const m=Math.min(r,gg,b); if(m<0){ r-=m; gg-=m; b-=m; } const mx=Math.max(r,gg,b)||1; return [r/mx,gg/mx,b/mx]; }
function waveColour(nm){ const rgb=fit(xyzToLin(xbar(nm),ybar(nm),zbar(nm))); const br=Math.min(1,Math.max(xbar(nm),ybar(nm),zbar(nm))/0.6); return hex(rgb.map(v=>v*br)); }
function xyColour(x,y){ if(y<=0) return '#000000'; const Y=1, X=x/y*Y, Z=(1-x-y)/y*Y; return hex(fit(xyzToLin(X,Y,Z))); }
function inSRGB(x,y){ const [r,g2,b]=[SRGB.r,SRGB.g,SRGB.b]; const sgn=(p,q,r2)=>(p[0]-r2[0])*(q[1]-r2[1])-(q[0]-r2[0])*(p[1]-r2[1]); const d1=sgn([x,y],r,g2), d2=sgn([x,y],g2,b), d3=sgn([x,y],b,r); const eps=1e-6; return !((d1<-eps||d2<-eps||d3<-eps)&&(d1>eps||d2>eps||d3>eps)); }
// the spectral locus: the fit between 420 and 640 nm, where it is good, closed with the observer's own tabulated points at 380, 660 and 700
const locus=[[0.1741,0.0050,380]]; for(let l=420;l<=640;l++){ const X=xbar(l),Y=ybar(l),Z=zbar(l), s=X+Y+Z; locus.push([X/s,Y/s,l]); } locus.push([0.7300,0.2700,660],[0.7347,0.2653,700]);
const locusAt=nm=>locus.find(p=>p[2]===nm);
function planckXY(T){ let X=0,Y=0,Z=0; for(let l=380;l<=780;l+=5){ const lm=l*1e-9; const B=1/(Math.pow(lm,5)*(Math.exp(1.438776877e-2/(lm*T))-1)); X+=B*xbar(l); Y+=B*ybar(l); Z+=B*zbar(l); } const s=X+Y+Z; return [X/s,Y/s]; }
const receptor=(r,nm)=>{ const [s1,s2]=r.w; const s=nm<r.peak?s1:s2; const t=(nm-r.peak)/s; return Math.exp(-0.5*t*t); };
...
function showLam(nm){ const rs=RECEPTORS.map(r=>[r,receptor(r,nm)]); const [S,M,L]=['s','m','l'].map(k=>receptor(RECEPTORS.find(r=>r.k===k),nm));
  card('Light at', nm+' nm, '+colourName(nm), [['looks like',sw(waveColour(nm))+waveColour(nm)],['S cones answer',fmt(S*100,0)+'% of their most'],['M cones',fmt(M*100,0)+'%'],['L cones',fmt(L*100,0)+'%'],['rods',fmt(receptor(RECEPTORS[3],nm)*100,0)+'%'],['the ratio L to M',M>0.01?fmt(L/M,2):'no M signal']],
...
function showXY(x,y){ const d=dominant(x,y); const inside=inSRGB(x,y);
  card('A colour', 'x = '+x.toFixed(3)+', y = '+y.toFixed(3), [['nearest a screen can show',sw(xyColour(x,y))+xyColour(x,y)],['dominant wavelength',d?(d.purple?'none: a purple, the complement of '+fmt(d.comp,0)+' nm':fmt(d.nm,0)+' nm, '+colourName(d.nm)):'none, this is white'],['purity',d?fmt(d.purity*100,0)+'% of the way from white to the spectrum':'0%'],['on a screen',inside?'inside the sRGB triangle: a screen can show it':'outside the triangle: no screen shows it, and the swatch above is a paler stand-in']],
    'The marker\\u2019s place on the horseshoe: its hue is the direction from the white point, and its purity how far along that line it lies toward the pure spectral colour at the edge.', 'Wikipedia, CIE 1931 color space; Wyman, Sloan & Shirley 2013'); }
function showPoint(k){ const p=POINTS.find(q=>q.k===k); card('A point on the map', esc(p.n), [['x, y',p.x.toFixed(4)+', '+p.y.toFixed(4)],['looks like',sw(xyColour(p.x,p.y))+xyColour(p.x,p.y)]], p.b, 'Wikipedia, Standard illuminant; Wikipedia, Planckian locus'); }
...
function showNames(){ card('The names','Eleven words, in order',[['first','black and white, in every language'],['then','red'],['then','green and yellow, either first'],['then','blue, then brown'],['last','purple, pink, orange and grey']],'Berlin and Kay found in 1969 that languages with few colour words have the same few, and that as words are added they come in nearly the same order everywhere, as if the spectrum were cut along the same lines by every people. Each swatch answers under the pointer.','Berlin & Kay 1969; Kay et al. 2009'); }

/* ---- the eye ---- */
const E={x:80,y:50,w:820,h:330,a:380,b:750};
const EX=nm=>E.x+(nm-E.a)/(E.b-E.a)*E.w;
function eyeView(){ let s='';
  for(let nm=E.a; nm<E.b; nm+=2) s+='<rect x="'+EX(nm).toFixed(1)+'" y="'+(E.y+E.h)+'" width="'+((E.w/((E.b-E.a)/2))+0.6).toFixed(2)+'" height="26" fill="'+waveColour(nm+1)+'"/>';
  s+='<rect x="'+E.x+'" y="'+E.y+'" width="'+E.w+'" height="'+E.h+'" fill="none" stroke="#2b2b2b"/>';
  for(const nm of [400,450,500,550,600,650,700,750]) s+='<text x="'+EX(nm).toFixed(1)+'" y="'+(E.y+E.h+42)+'" text-anchor="middle" font-size="10.5" fill="#9a9a9a">'+nm+'</text>';
  s+='<text x="'+(E.x+E.w/2)+'" y="'+(E.y+E.h+60)+'" text-anchor="middle" font-size="11" fill="#9a9a9a">wavelength, nm</text>';
  s+='<text transform="translate(18,'+(E.y+E.h/2)+') rotate(-90)" text-anchor="middle" font-size="11" fill="#9a9a9a">sensitivity, each at its own peak</text>';
  for(const r of RECEPTORS){ let d=''; for(let nm=E.a; nm<=E.b; nm+=1){ const v=receptor(r,nm); d+=(d?'L':'M')+EX(nm).toFixed(1)+','+(E.y+E.h-v*(E.h-30)).toFixed(1); } const on=hot===r.k;
    s+='<path data-rec="'+r.k+'" d="'+d+'" fill="none" stroke="'+r.c+'" stroke-width="'+(on?3.2:2.2)+'"'+(r.k==='rod'?' stroke-dasharray="6 4"':'')+' style="cursor:pointer"/>';
    s+='<text x="'+EX(r.peak).toFixed(1)+'" y="'+(E.y+18)+'" text-anchor="middle" font-size="11" fill="'+r.c+'">'+(r.k==='rod'?'rods':r.k.toUpperCase())+' '+r.peak+'</text>'; }
  const mx=EX(lam);
...
/* ---- the gamut ---- */
const P={x0:100,y0:600,s:640};
const PX=x=>P.x0+x*P.s, PY=y=>P.y0-y*P.s;
const IX=px=>(px-P.x0)/P.s, IY=py=>(P.y0-py)/P.s;
let fill=null;
function inHorseshoe(x,y){ let inside=false; const n=locus.length; for(let i=0,j=n-1;i<n;j=i++){ const xi=locus[i][0],yi=locus[i][1],xj=locus[j][0],yj=locus[j][1]; if((yi>y)!==(yj>y)&&x<(xj-xi)*(y-yi)/(yj-yi)+xi) inside=!inside; } return inside; }
function makeFill(ctx){ const w=W, h=640; const img=ctx.createImageData(w,h); const d=img.data;
  for(let py=0;py<h;py++) for(let px=0;px<w;px++){ const x=IX(px+0.5), y=IY(py+0.5); const i=(py*w+px)*4; if(y>0&&inHorseshoe(x,y)){ const c=xyColour(x,y); d[i]=parseInt(c.slice(1,3),16); d[i+1]=parseInt(c.slice(3,5),16); d[i+2]=parseInt(c.slice(5,7),16); d[i+3]=255; } else { d[i]=18; d[i+1]=18; d[i+2]=18; d[i+3]=255; } }
  return img; }
function gamutView(){ const cv=document.createElement('canvas'); cv.width=W; cv.height=640; cv.id='gcanvas'; const ctx=cv.getContext('2d');
  ...
  // the axes
  ctx.strokeStyle='#2b2b2b'; ctx.fillStyle='#9a9a9a'; ctx.font='10.5px sans-serif'; ctx.textAlign='center';
  for(let v=0;v<=0.8;v+=0.1){ ctx.beginPath(); ctx.moveTo(PX(v),P.y0); ctx.lineTo(PX(v),P.y0+6); ctx.stroke(); ctx.fillText(v.toFixed(1),PX(v),P.y0+20); }
  ctx.textAlign='right'; for(let v=0;v<=0.9;v+=0.1){ ctx.beginPath(); ctx.moveTo(P.x0-6,PY(v)); ctx.lineTo(P.x0,PY(v)); ctx.stroke(); ctx.fillText(v.toFixed(1),P.x0-10,PY(v)+4); }
  ctx.textAlign='center'; ctx.fillText('x',PX(0.4),P.y0+36); ctx.save(); ctx.translate(P.x0-40,PY(0.45)); ctx.rotate(-Math.PI/2); ctx.fillText('y',0,0); ctx.restore();
  // the locus and its wavelengths
  ctx.strokeStyle='#e6e6e6'; ctx.lineWidth=1.2; ctx.beginPath(); locus.forEach((p,i)=>{ i?ctx.lineTo(PX(p[0]),PY(p[1])):ctx.moveTo(PX(p[0]),PY(p[1])); }); ctx.closePath(); ctx.stroke();
  ctx.fillStyle='#e6e6e6'; ctx.font='10.5px sans-serif';
  for(const nm of [420,460,470,480,490,500,510,520,540,560,580,600,620,700]){ const p=locusAt(nm); const cx=PX(0.33),cy=PY(0.33); const dx=PX(p[0])-cx, dy=PY(p[1])-cy, L=Math.hypot(dx,dy); const tx=PX(p[0])+dx/L*16, ty=PY(p[1])+dy/L*16; ctx.beginPath(); ctx.arc(PX(p[0]),PY(p[1]),2.2,0,7); ctx.fill(); ctx.textAlign=dx<0?'right':'left'; halo(String(nm),tx+(dx<0?4:-4),ty+4); }
  ...
  ctx.fillStyle='#9a9a9a'; ctx.font='11px sans-serif'; ctx.textAlign='left'; ctx.fillText('every colour the eye can see, as the CIE 1931 diagram: the curved edge is the pure spectrum,',PX(0.22),PY(0.86));
...
function namesView(){ let s=''; const x0=90, cw=118, y0=70, sw2=78, sh=58, gap=30;
  ...
  s+='<text x="'+x0+'" y="'+(H-10)+'" font-size="11" fill="#9a9a9a">the eleven basic colour terms of Berlin and Kay, in the order languages acquire them; a column is one stage, and within the last the order varies</text>';
  return {svg:s, h:H}; }
...
function render(){ if(view==='gamut'){ el.innerHTML=''; el.appendChild(gamutView()); } else { const q=view==='eye'?eyeView():namesView(); el.innerHTML='<svg viewBox="0 0 '+W+' '+q.h+'" xmlns="http://www.w3.org/2000/svg" id="csvg"><rect width="'+W+'" height="'+q.h+'" fill="#121212"/>'+q.svg+'</svg>'; }
  document.getElementById('eyeCtl').hidden=view!=='eye'; document.getElementById('gamutCtl').hidden=view!=='gamut'; document.getElementById('lamOut').textContent=lam+' nm, '+colourName(lam); document.getElementById('xyOut').textContent='x '+mk.x.toFixed(3)+', y '+mk.y.toFixed(3); }
...
render(); showEye();
window.__colour=(q)=>{ const o={view,hot,lam,mk,card:document.getElementById('numTxt').innerText,name:document.getElementById('nameTxt').innerText,body:document.getElementById('bodyTxt').innerText,
  ...
  if(q&&q.nm!=null){ o.resp=RECEPTORS.map(r=>receptor(r,q.nm)); o.ex=EX(q.nm); o.col=waveColour(q.nm); }
  if(q&&q.xy){ const [x,y]=q.xy; o.dom=dominant(x,y); o.inside=inSRGB(x,y); o.hex=xyColour(x,y); o.inHorse=inHorseshoe(x,y); o.px=[PX(x),PY(y)]; }
  if(q&&q.T!=null) o.planck=planckXY(q.T);
  if(q&&q.locus) o.locus=locus.filter(p=>q.locus.includes(p[2])).map(p=>[p[0],p[1],p[2]]);
  o.locusN=locus.length;
  return o; };
</script>
</body>
</html>
