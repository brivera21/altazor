...
NOTE2 = ("The named milestones above the line carry their dating and its "
         ...
         "strands below hold the detail, from a Big History notebook.")
...
.bar2 { display:flex; align-items:center; gap:10px; flex-wrap:wrap; margin:0 0 10px; }
.bar2 input { background:var(--panel); color:var(--text); border:1px solid var(--line);
  border-radius:8px; padding:6px 10px; width:230px; font:inherit; font-size:13.5px; }
.bar2 button { background:var(--panel); color:var(--muted); border:1px solid var(--line);
  border-radius:999px; padding:5px 11px; font-size:12.5px; cursor:pointer; font-family:inherit; }
.bar2 button:hover:not(:disabled) { color:var(--text); border-color:#3d3d3d; }
.bar2 button:disabled { opacity:.4; cursor:default; }
.bar2 .sep { flex:1 1 0; }
...
<details><summary>What the notebook said, and what the line says now</summary>
__FIXED__
</details></div>
...
let dragX=null, dragX2=null;  // the band being pulled out over the line
let finding=false, found={ev:new Set(),det:new Set()}, hits=[], hitAt=-1;
...
function u(t){
  ...
  if(mode==='sun'){
    ...
    const v=Math.log10(Math.max(TMIN,t));
    if(v>=LSUN) return (LMAX-v)/(LMAX-LSUN)*0.5;
    return 0.5+(LSUN-v)/(LSUN-LMIN)*0.5;
  }
  const v=Math.log10(Math.max(TMIN,t));   // log years before now
  return (LMAX-v)/(LMAX-LMIN);
}
let win=null;         // [older, newer] in years before now, or the whole line
function X(t){
  const w=W-L-R, p=u(t);
  if(!win) return L+p*w;
  const a=u(win[0]), b=u(win[1]);
  return L+(p-a)/(b-a)*w;
}
function inView(x){ return x>=L-2 && x<=W-R+2; }
...
// nice round values inside a range: powers of ten and their halves and
// fifths on a log reading, an even step on a linear one
function niceVals(lo,hi,linear){
  const out=[];
  if(linear){
    const raw=(hi-lo)/6, e=Math.pow(10,Math.floor(Math.log10(raw)));
    const m=raw/e, step=(m<1.5?1:m<3.5?2:m<7.5?5:10)*e;
    for(let v=Math.ceil(lo/step)*step; v<=hi+step*1e-9; v+=step) out.push(v);
    return out;
  }
  // widen the set of round numbers until the span carries enough of them
  for(const mants of [[1,2,5],[1,1.5,2,3,5,7],[1,1.2,1.5,2,2.5,3,4,5,6,7,8,9]]){
    out.length=0;
  ...
  out.sort((a,b)=>a-b);
  return out.length>11 ? out.filter(v=>{
    const l=Math.log10(v); return Math.abs(l-Math.round(l))<1e-9; }) : out;
}
// the ticks for the line as it stands, in years before now
function ticksNow(){
  if(!win) return TICKS[mode].filter(t=>t<=NOW&&t>=0)
    .map(t=>({t:mode==='fwd'?NOW-t:t, lab:tickLabel(t)}));
  const [o,n]=win;
  if(mode==='fwd'){                       // read as years since the Big Bang
    return niceVals(Math.max(1,NOW-o),NOW-n,false)
      .map(a=>({t:NOW-a, lab:unit(a)}));
  }
  const lin=mode==='even';
  return niceVals(lin?n:Math.max(TMIN,n), o, lin)
    .map(t=>({t, lab:t===0?'now':unit(t)}));
}
...
function packLanes(){
  const lanes=[], laneOf={};
  const order=EV.map((e,i)=>({e,i,x:X(e.t)})).filter(o=>inView(o.x))
    .sort((a,b)=>a.x-b.x);
...
function render(){
  ...
  shown.forEach((st,r)=>{
    ...
    DET.forEach((d,i)=>{
      ...
      const hot=i===curDet, hit=found.det.has(i), dull=finding&&!hit;
      s+='<g data-d="'+i+'" style="cursor:pointer">'
        +(hit?'<circle cx="'+x.toFixed(1)+'" cy="'+mid.toFixed(1)
            +'" r="7.5" fill="none" stroke="#f4efe2" stroke-width="1.4" stroke-opacity="0.85"/>':'')
        +'<circle cx="'+x.toFixed(1)+'" cy="'+mid.toFixed(1)+'" r="'+(hot?5:2.9)
        +'" fill="'+st.c+'" fill-opacity="'+(hot?1:dull?0.16:0.72)+'"'
  ...
  s+='<text x="'+(W-R)+'" y="'+(AXIS+38)+'" text-anchor="end" font-size="10.5" fill="#6b7280">'
    +CAP[mode]+'</text>';
  s+='<text x="'+L+'" y="20" font-size="11" fill="#6b7280">'
    +(win?ago(win[0]):'the Big Bang')+'</text>'
    +'<text x="'+(W-R)+'" y="20" text-anchor="end" font-size="11" fill="#6b7280">'
    +(win?ago(win[1]):'now')+'</text>';

  // the milestones, above the line
  EV.forEach((e,i)=>{
    const x=X(e.t);
    if(!inView(x)||pack.laneOf[i]===undefined) return;
    ...
    const ehit=found.ev.has(i);
    s+='<path d="M'+x.toFixed(1)+','+y+' V'+AXIS+'" stroke="'+c+'" stroke-width="1" stroke-opacity="'
      +(finding&&!ehit?0.15:0.45)+'"/>'
      +(ehit?'<circle cx="'+x.toFixed(1)+'" cy="'+y
          +'" r="10" fill="none" stroke="#f4efe2" stroke-width="1.4" stroke-opacity="0.85"/>':'')
      +'<circle cx="'+x.toFixed(1)+'" cy="'+y+'" r="'+(i===cur&&curDet<0?6:4.5)+'" fill="'+c
      +'" fill-opacity="'+(finding&&!ehit?0.3:1)+'" stroke="#121212" stroke-width="1.4"/>'
  ...
  // the band being dragged out, and the matches a search has turned up
  if(dragX!==null){
    const x0=Math.min(dragX,dragX2), x1=Math.max(dragX,dragX2);
    s+='<rect x="'+x0.toFixed(1)+'" y="'+(AXIS-14)+'" width="'+(x1-x0).toFixed(1)
      +'" height="'+(H-AXIS+6)+'" fill="#58a6ff" fill-opacity="0.13" '
      +'stroke="#58a6ff" stroke-opacity="0.6" stroke-width="1"/>';
  }
  s+='</svg>';
  el.innerHTML=s;
}
...
function toggle(k){
  ...
  if(finding) runFind(findBox.value); else render();
}
...
/* ---- the window: a drag over the line rescales it to that span ---- */
// the inverse of u(t), so a pixel can be read back as a date
function tAt(px){
  const w=W-L-R;
  let p=(px-L)/w;
  if(win){ const a=u(win[0]), b=u(win[1]); p=a+p*(b-a); }
  p=Math.min(1,Math.max(0,p));
  if(mode==='even') return NOW*(1-p);
  if(mode==='fwd') return NOW-Math.pow(10,p*Math.log10(NOW));
  if(mode==='sun') return p<=0.5 ? Math.pow(10,LMAX-p*2*(LMAX-LSUN))
                                 : Math.pow(10,LSUN-(p-0.5)*2*(LSUN-LMIN));
  return Math.pow(10,LMAX-p*(LMAX-LMIN));
}
function winTxt(){
...
function setWin(w){
  if(w){
    const o=Math.min(NOW,Math.max(w[0],w[1])), n=Math.max(0,Math.min(w[0],w[1]));
    // a span narrower than a hair of the line is a click, not a window
    if(Math.abs(u(o)-u(n))<0.004){ win=null; } else win=[o,n];
  } else win=null;
  winTxt(); render();
}
document.getElementById('whole').onclick=()=>setWin(null);

function svgX(ev){
  const svg=document.getElementById('tsvg'), r=svg.getBoundingClientRect();
...
el.addEventListener('pointerdown',ev=>{
  if(svgY(ev)<AXIS-14) return;        // the milestones above the line stay clickable
  dragX=dragX2=Math.min(W-R,Math.max(L,svgX(ev)));
  el.setPointerCapture&&el.setPointerCapture(ev.pointerId);
  ev.preventDefault();
});
el.addEventListener('pointermove',ev=>{
  if(dragX===null) return;
  dragX2=Math.min(W-R,Math.max(L,svgX(ev)));
  render();
});
window.addEventListener('pointerup',ev=>{
  if(dragX===null) return;
  const a=dragX, b=dragX2;
  dragX=dragX2=null;
  if(Math.abs(a-b)<6){ render(); return; }   // a click on a row, not a drag
  setWin([tAt(Math.min(a,b)),tAt(Math.max(a,b))]);
});
...
function runFind(q){
  q=(q||'').trim();
  finding=q.length>0;
  found={ev:new Set(),det:new Set()}; hits=[]; hitAt=-1;
  if(finding){
    // a match is a word that starts with what was typed, so that "war"
    // finds warfare but not toward
    const ql=q.toLowerCase();
    const hit=n=>{
      const t=n.toLowerCase();
      let at=t.indexOf(ql);
      while(at>=0){
        if(at===0 || !/[a-z0-9]/.test(t.charAt(at-1))) return true;
        at=t.indexOf(ql,at+1);
      }
      return false;
    };
    EV.forEach((e,i)=>{ if(hit(e.n)){
      found.ev.add(i); hits.push(['ev',i,e.t]); } });
    DET.forEach((d,i)=>{ if(hit(d.n)&&!off.has(d.k)){
      found.det.add(i); hits.push(['det',i,d.t]); } });
...
function step(){
  ...
  const [kind,i,t]=hits[hitAt];
  // a match outside the window slides the window along to hold it
  if(win && (t>win[0]||t<win[1])){
    const half=Math.abs(u(win[0])-u(win[1]))/2, p=u(t);
    win=[tOfU(Math.max(0,p-half)), tOfU(Math.min(1,p+half))];
    winTxt();
  }
  if(kind==='ev') show(i); else showDet(i);
  document.getElementById('findTxt').textContent=
    (hitAt+1)+' of '+hits.length+(hits.length===1?' match':' matches');
}
// a position along the whole line, read back as a date
function tOfU(p){
  const keep=win; win=null;
  const t=tAt(L+p*(W-L-R));
  win=keep; return t;
}
document.getElementById('fitFind').onclick=()=>{
  if(!hits.length) return;
  const ts=hits.map(h=>h[2]);
  let o=Math.max(...ts), n=Math.min(...ts);
  const pad=(u(n)-u(o))*0.08||0.02;
  setWin([tOfU(Math.max(0,u(o)-pad)), tOfU(Math.min(1,u(n)+pad))]);
};
...
window.__cosmic=()=>({events:EV.length, detail:DET.length,
  ...
  win, hits:hits.length, finding,
  ticks:[...document.querySelectorAll('#tsvg text')].length});
</script>
</body>
</html>
"""
