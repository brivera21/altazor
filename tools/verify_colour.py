"""Check colour.html against its data, against the CIE tables, and against
its drawing.
...
from colour_data import RECEPTORS, COUNTS, SRGB, POINTS, TERMS, STAGES

ROOT = Path(__file__).resolve().parent.parent
PAGE = ROOT / "colour.html"
fails = []
...
check(COUNTS["cones"] == 4.6e6 and COUNTS["rods"] == 92e6 and COUNTS["fovea_per_mm2"] == 199000, "4.6 million cones, 92 million rods, 199,000 a square millimetre")
...
with sync_playwright() as pw:
    ...
    def st(q=None):
        return pg.evaluate("(q)=>window.__colour(q)", q)
