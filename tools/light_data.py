...
# k, name, from (m), to (m), colour, a line
BANDS = [
    ...
    ("visible", "visible light", 3.8e-7, 7.5e-7, "#f4efe2",
     "Less than one octave, 380 to 750 nanometres, where the Sun's output peaks and the air is clear. Eyes evolved into this gap."),
    ("ir", "infrared", 7.5e-7, 1e-3, "#ffb02e",
     "Heat radiation. Everything at room temperature glows here, peaking near ten microns; water vapour absorbs most of it."),
    ("micro", "microwaves", 1e-3, 1e-1, "#e0a458",
     "Millimetres to a decimetre: radar, ovens, Wi-Fi, and the afterglow of the Big Bang, which peaks at a millimetre."),
    ("radio", "radio waves", 1e-1, 1e4, "#58a6ff",
     "From a decimetre up. Broadcasting, and the 21 cm line of hydrogen that maps the galaxy. Above ten metres or so the ionosphere reflects it back."),
]
...
MARKS = [
    ...
    ("uvc", "germicidal ultraviolet", 2.54e-7, "the 254 nm mercury line",
     "Breaks DNA, which is why lamps at this wavelength sterilise, and why the ozone layer, which stops it, matters.", "Wikipedia, Ultraviolet germicidal irradiation"),
    ...
    ("sodium", "the sodium D line", 5.893e-7, "589 nm, orange",
     "The yellow-orange of old street lamps and of salt in a flame: two lines a fraction of a nanometre apart, the strongest in the Sun's spectrum after hydrogen.", "Wikipedia, Sodium-vapor lamp"),
    ...
    ("fibre", "light in an optical fibre", 1.55e-6, "1550 nm",
     "The wavelength glass is clearest at, so undersea cables use it: a pulse goes a hundred kilometres before it needs boosting.", "Wikipedia, Optical fiber"),
    ...
    ("wifi", "Wi-Fi", 6.0e-2, "5 GHz, 6 cm",
     "The higher of the two common bands. A wavelength of centimetres passes doors and reflects off walls; the router's antennas are a quarter of it long.", "Wikipedia, Wi-Fi"),
    ...
    ("fm", "FM radio", 3.0, "100 MHz, 3 m",
     "A wavelength of three metres, which is why a car aerial is about 75 cm, a quarter wave. Goes to the horizon and little further.", "Wikipedia, FM broadcasting"),
...
BODIES = [
    ...
    ("zetapup", "Zeta Puppis", 40000, "One of the hottest naked-eye stars. The visible band catches a sliver of the total; the colour has stopped changing, a saturated blue-white.", "Wikipedia, Zeta Puppis"),
]
...
REFS = [
    ...
    (apa.article("Wyman, C., Sloan, P.-P., &amp; Shirley, P.", 2013,
     ...
     "The colour of each wavelength and of each hot body, from the eye's three matching functions."),
]
REFS += [(apa.wiki(f"https://en.wikipedia.org/wiki/{p}"), a) for p, a in [
    ("Electromagnetic_spectrum", "The bands and their edges."),
    ("Visible_spectrum", "380 to 750 nanometres."),
    ...
    ("SRGB", "Turning the eye's three responses into a colour on the screen."),
