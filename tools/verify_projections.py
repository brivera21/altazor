...
with sync_playwright() as pw:
    ...
    s = st()
    check(s["proj"] == "ortho" and "Hipparchus" in s["card"] and not pg.evaluate("()=>document.getElementById('centreCtl').hidden"), "the orthographic globe shows its centre buttons")
    pg.click('#centres button[data-c="pac"]')
    pg.wait_for_timeout(300)
    s = st()
    check(s["centre"] == "pac" and "cut by the edge" in s["card"], "turned to the Pacific, the cap at 20 E is over the edge")
