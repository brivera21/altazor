...
METHOD = ("Frequencies are twelve-tone equal temperament on A4 = 440 Hz, "
          ...
          "to 443. Wavelengths use 343 metres a second, the speed of sound "
...
function showFreq(f){ const m=freqMidi(f), n=Math.round(m), off=Math.round((m-n)*100); const lam=C_AIR/f;
  ...
  card('A pitch', fmt(f,f<100?1:0)+' Hz, '+(off?noteName(n)+' '+(off>0?'+':'')+off+' cents':noteName(n)), [['wavelength in air',lam>=1?fmt(lam,2)+' m':fmt(lam*100,1)+' cm'],['period',f<1000?fmt(1000/f,2)+' ms':fmt(1e6/f,0)+' \\u00b5s'],['on the piano',n>=PIANO[0]&&n<=PIANO[1]?'key '+(n-PIANO[0]+1)+' of 88':n<PIANO[0]?'below the lowest key':'above the highest key'],['harmonics',harm.join('; ')],['within the range of',within.length?within.join(', '):'no voice or instrument listed here']],
    'A string or a column of air sounding this pitch also sounds its whole-number multiples, fainter: the octave, then the fifth above it, then the second octave, then the major third above that. The ear hears them as one note with a colour.', 'Wikipedia, Harmonic series (music); Wikipedia, Piano key frequencies'); }
...
function showJust(){ card('The intervals','Pure against tempered',[['a fifth, pure','3:2, 701.96 cents'],['a fifth, on the piano','700 cents, 2 short'],['a major third, pure','5:4, 386.31 cents'],['a major third, on the piano','400 cents, 14 wide']],'Each grey dot is one of the piano\\u2019s twelve equal steps; the coloured dot beside it is the pure ratio, and the little line between them is how far the piano is out. The fifths are nearly perfect; the thirds are not.','Wikipedia, Just intonation; Wikipedia, Equal temperament'); }
