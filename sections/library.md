---
title: Library
---

## Digital Concepts

Nothing here yet.

## Mexico

[Roads Around El Terrero, Namiquipa](el-terrero.html). The towns of the Namiquipa valley in Chihuahua and the municipal seats around them, each at its real coordinates and joined only where a road exists, with the driving distance along every link. Population from the 2020 census, distances routed over OpenStreetMap. The page is in Spanish.

## USA

[Every State by Population](us-states.html). All fifty states in order, largest to smallest, with each one's share of the country and its change since the 2020 census, from Census Bureau estimates.

[Most Populous Cities in the United States](us-cities.html). The twenty largest cities in 2025: population, share of the country, and how much each has grown or shrunk since the 2020 census, from Census Bureau estimates.

## Demography

[World's Most Populous Countries](populous-countries.html). The twenty largest countries in 2026: population, share of the world total, and births and deaths per day, from the United Nations projections.
